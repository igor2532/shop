...
function Comments() {
  ...
  const addComment = (e) => {
    ...
    if (
      ...
      setComments([
        ...comments,
        { user: nameValue, title: textValue, idProduct: params.id, likes: 0 },
      ]);
    }
  };
  return (
    <div className="App_comments">
        ...
        {arrComment.map((item, itemKey) => (
          <div className="App_comments_items_item">
            <div className="App_comments_items_block_up">
              <div className="App_comments_items_item_author">
                {" "}
                {item.user}{" "}
              </div>
              <div className="App_comments_items_item_date">
                {" "}
                {item.date} {item.time}{" "}
              </div>
            </div>
            <div className="App_comments_items_item_title"> {item.title} </div>
            <div className="App_comments_items_item_likes">
              {" "}
              {item.likes}{" "}
              <button>
      ...
      <div>
        <form className="App_form">
          ...
          <div>
            <button onClick={addComment} className="orderBtn">
              Отправить
            </button>
          </div>
        </form>
